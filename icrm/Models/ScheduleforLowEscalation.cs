...
{
    public class ScheduleforLowEscalation : IJob, IRegisteredObject
    {
        ...
        public ScheduleforLowEscalation()
        {
                ...
                lock (_lock)
                {
                    ...
                    var level1query = from f in db.Feedbacks.ToList()
                                where f.assignedDate != null && f.checkStatus == Constants.ASSIGNED && f.type.name == Constants.Complaints &&
                                 f.priority.priorityId == 4 && f.escalationlevel == null && (DateTime.Now- (DateTime)f.assignedDate).TotalHours > Constants.lowescelationtime &&
                                (DateTime.Now - (DateTime)f.assignedDate).TotalHours < (Constants.lowescelationtime) *2
                                      select f;
                    ...
                    var level2query = from f in db.Feedbacks.ToList()
                                where f.assignedDate != null && f.checkStatus == Constants.ASSIGNED && f.type.name == Constants.Complaints &&
                                 f.priority.priorityId == 4 && f.escalationlevel == "level1" && (DateTime.Now - (DateTime)f.assignedDate).TotalHours > (Constants.lowescelationtime) *2 &&
                                (DateTime.Now - (DateTime)f.assignedDate).TotalHours < (Constants.lowescelationtime) * 3
                                      select f;
                    ...
                    //                   f.priority.priorityId == 4 && f.escalationlevel == "level2" && (DateTime.Now - (DateTime)f.assignedDate).TotalHours > (Constants.lowescelationtime) * 3
                    //                  select f;
